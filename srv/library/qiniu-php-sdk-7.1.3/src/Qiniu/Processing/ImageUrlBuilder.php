...
final class ImageUrlBuilder
{
    ...
    {
        return $this->water(self::WATER_IMG, $url) . $this->waterImg(
            $image,
            $dissolve,
             $gravity,
             $dx,
    ...
    {
        return $this->water(self::WATER_TEXT, $url) . $this->waterText(
            $text,
            $font,
            $fontSize,
            $fontColor,
    ...
    public function water($type, $url)
    {
        // url合法效验
        if (!$this->isUrl($url)) {
            return $url;
        ...
        switch ($type) {
            case self::WATER_IMG:
                return $this->queryUrl($url) . 'watermark/1';
                break;
            case self::WATER_TEXT:
                return $this->queryUrl($url) . 'watermark/2';
            case self::WATER_IMG_TEXT:
                return $this->queryUrl($url) . 'watermark/3';
            default:
                return null;
        }
    ...
    {
        $waterStr = '/image/' . \Qiniu\base64_urlSafeEncode($image) . '/';
