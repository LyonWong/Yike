...
{
    protected $scopeKey = 'ticket-refund';
    ...
    public function _GET_data()
    {
        ...
        $res = servTicket::sole($this->platform)->page(dataTicket::TYPE_REFUND_APPEAL,$query, round(($start/$length)+1), $length);
        foreach ($res['pages'] as &$item) {
            ...
            $item = [
                ...
                $item['refundInfo']['appeal']['reason'],
                $item['refundInfo']['appeal']['tms_create'],
                $item['refundInfo']['appeal']['remark'],
                $item['refundInfo']['appeal']['tms_update'],
                $item['refundInfo']['appeal']['status'],
    ...
    public function _POST_operate()
    {
        ...
        $ret = servRefund::sole($this->platform)->dealApply($this->uid, $remark, $operate, $ticketId);
        if ($ret) {
            $this->apiSuccess();
        }
