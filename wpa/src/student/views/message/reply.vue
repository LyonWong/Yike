<template>
  <section class="dicuss-reply">
    <loading :show="showLoading"></loading>
    <div class="container">
      <div class="title clearfix">
        <div class="pull-left" v-if="genre == 2">
          <span class="answer">回复：</span>
          <span v-text="name"></span>
        </div>
        <div class="pull-left" v-if="genre == 1">
          <span class="answer">留言</span>
        </div>
        <div class="pull-right">
          <span class="close" @click="closeReply">
      ...
      <div class="img-list clearfix" v-if="curImage.length || data.file">
        ...
        <div class="img" v-if="data.file">
          <span class="img-wrap">
            {{data.file | lastPathName}}
          </span>
      ...
      <div class="bottom clearfix">
        ...
        <div class="pull-left">
          <span class="iconfont icon-fujiawenjian">
...
<script>
  ...
  export default{
    ...
    methods: {
      closeReply() {
        this.$parent.showReply = false;
        let contentMessage = document.querySelector('.content-message');
        if(contentMessage && contentMessage.scrollTop > 0){
      ...
      boardReply() {
        // 文字不能为空
        if(!this.data.text && !this.data.image.length){
          return swal({
            title: '错误提醒',
            text: '请输入留言或添加图片!',
        ...
        this.$store.dispatch('fetchBoardReply', {...this.data}).then((data) => {
          // 回调
          this.$emit('replyCallback', data);
          this.closeReply();
      ...
      boardComment() {
        ...
        // 开始发送留言
        this.showLoading = true;
        this.$store.dispatch('fetchBoardComment', {...this.data}).then((data) => {
          // 保存当前排序
          this.$store.commit('CHANGE_MESSAGE_TYPE', {[this.data.type]:'time_desc'});
          // 获得列表
          this.$router.push({ name: 'messageDiscuss', params: { lesson_sn: this.data.lesson_sn }, query: {type_now:this.data.type,timestamp:Date.parse(new Date())}});
          this.closeReply();
          this.showLoading = false;
        }, (error) => {
          //
      ...
      fileOnChange(event) {
          ...
          // 开始上传
          var the = this;
          this.uploadDraft(file, (data)=>{
              the.data.file = data.key;
          });
          uploadFiles.value = '';
      },
      // 获得权限
      ...
      uploadDraft(file, callback) {
          console.log('file', file);
        this.showLoading = true;
        this.$store.dispatch('fetchBoardDraft', {name: file.name}).then((data) => {
            var formData = new FormData();
            formData.append('file', file);
            formData.append('key', data.key);
            formData.append('token', data.token);
