...
export const fetchRankLocate = ({commit}, query) => {
  const url = `${_prefix}/promote-rank-locate.api`;

  return _get({ url, query }, commit)
    .then((json) => {
      if (json.error == 0) {
        return commit('FETCH_RANK_LOCATE', json.data)
      }
...
export const fetchRankList = ({commit}, query) => {
