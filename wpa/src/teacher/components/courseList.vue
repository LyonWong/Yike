    ...
    <ul>
      ...
      <li v-for="list in lists">
        ...
        <div class="handle-btn">
          <a class="handle-item" href="javascript:;" v-if="list.i_form==2" :title="(list.i_step===1) ? '开始备课' : '查看内容'" @click="checkView(list)">
            <i class="iconfont icon-yuanchengshouke"></i>
          </a>
          <a class="handle-item" href="javascript:;" :title="(list.step=='opened')?(checkCanOpen(list) ? '点击开课':'点击备课'):'进入直播'" @click="checkTime(list)" v-if="list.i_form !== 2 && list.step != 'submit' && list.step != 'denied'">
            <i class="iconfont icon-yuanchengshouke"></i>
          </a>
...
<script>
    ...
    let host = process.env.NODE_ENV == 'production' ? process.env.LIVE_HOST : '/';
    ...
    export default{
      ...
      methods: {
        ...
        checkView(list) {
          if (list.i_step === 1) {
            this.enterPrepareLesson(list);
          } else {
            window.open(`${host}study/view/${list.sn}`);
          }
        }
      },
    };
