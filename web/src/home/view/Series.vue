<template>
  <div class="v-series">
    <div class="profile">
      ...
      <detail-policy v-if="policy.hints.length" :policy="policy"></detail-policy>
    </div>
    ...
    <div id="catalog">
      <div id="category" class="relative border" v-if="relative.length">
        <block title="目录">
          <lesson-cell :home="home" :lesson="lessonStatus(item)" v-for="(item,index) in relative" :key="index"></lesson-cell>
        </block>
      </div>
    </div>
    <div class="invite">
    ...
    <div class="contact" v-if="conf.inform">
    ...
    <div class="control flex-row" v-if="individual">
      ...
      <div class="ctrl-locked ctrl-text font-medium" v-if="canEnroll && app.os() === 'iOS' && app.env() === 'wxa'">小程序iOS端暂不能支付</div>
    ...
    <modal-action v-on:close="displayAfterEnroll = false" :display="displayAfterEnroll" width="7rem" v-if="individual">
      ...
      <ul>
        <li v-for="(item, idx) in policy.items" :key="idx">
          {{item.desc}}
        </li>
      </ul>
    ...
    <modal-action v-on:close="displayBeforeActivity = false" :display="displayBeforeActivity" width="7rem" v-if="individual && conf.activity && conf.inform">
...
<script>
  ...
  import DetailCover from '@/lesson/components/DetailCover'
  import DetailTeacher from '@/lesson/components/DetailTeacher'
  import DetailPolicy from '@/lesson/components/DetailPolicy'
  import DetailInvite from "@/lesson/components/DetailInvite";
  import DetailContact from '@/lesson/components/DetailContact'
  import DetailOrder from '@/lesson/components/DetailOrder'
  import SeriesRefund from "@/lesson/components/SerieslRefund"
  ...
  // import ModalAction from "@/lesson/@/lesson/components/modal/Action"
  ...
  export default {
    ...
    created() {
      console.log('sn', this.sn, this.home)
      ...
      this.api.get('/api/series-conf', {
        ...
        this.initializing--
        this.conf = res.data
        if (this.conf.refundable === false) {
          this.policy.hints.push('不可退款')
        } else {
          this.policy.hints.push('一小时退款')
          this.policy.items.push({
            head: '一小时退款',
            desc: '自开始听课起，一小时内可无条件退款，退款后不可再次购买'
          })
        }
        if (this.conf.inform) {
          this.policy.hints.push('开课通知')
          let href = this.app.env() === 'wxm' ? this.app.config.mpProfile : this.app.linkToAssets('/img/qrcode/yike-fm.png')
          this.policy.items.push({
            head: '开课通知',
            desc: `关注【易灵微课】公众号<a id="qrcode" style="color: #2F57DA;cursor: pointer" target="_blank" href="${href}">yike-fm<a/>，可接收开课通知`
          })
        }
        if (this.conf.activity) {
          this.conf.activity.href += `&origin=${this.origin}`
        }
      })
    },
    mounted() {
    ...
    methods: {
      ...
      completeEnroll() { // 订单支付完成
        ...
        if (this.conf.refundable === false) { // 自动确认不可退款的课程
          this.api.post('/api/order-confirm-series', {
            sn: this.sn
          })
        }
        // 重新获取课程状
      ...
      activity() { // 邀请卡
        ...
        if (this.individual.subscribed || !this.conf.inform) {
