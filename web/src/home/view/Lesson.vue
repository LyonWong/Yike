<template>
  <div class="v-detail">
    ...
    <div class="control flex-row" v-if="individual">
      ...
      <div class="ctrl-refund ctrl-text font-medium" @click="refund" v-if="individual.refund && conf.refundable!==false">退款</div>
  ...
  export default {
    ...
    methods: {
      ...
      completeEnroll() { // 订单支付完成
        this.order = null
        if (this.conf.refundable === false) { // 不可退款的课程，购买后自动确认
          this.api.post('/api/order-confirm-lesson', {
            sn: this.sn
          })
        }
        // 重新获取课程状态
      ...
      go(index) {
        if (this.home) {
          location.href = `/home/${this.sn}/series?sn=${this.profile.series.sn}`
        } else {
